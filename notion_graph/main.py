...
if __name__ == '__main__':
    try:
        ...
        output = sys.argv[3]
        notion_db_perso = NotionDatabase(token=token, database_id=database_id)
        notion_db_perso.show_graph(output=output)
        print(f"results in {output}")
    except Exception as e:
        print(f"Exception raised {e}") 
        print("Did you use this script with poetry run python -m notion_graph.main {secret_token} {database_id} {output}")
